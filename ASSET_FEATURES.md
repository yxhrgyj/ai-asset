...
## 直接发布与安全上传（2026-09-08）

- 作者可以直接发布自己的草稿，管理员可以代为发布。纯附件资产也可以发布；创建新版本会继承正文和附件。
- 审核入口和接口已移除。V8 迁移将历史待审及最新驳回版本恢复为草稿，将审核角色转为作者，历史审核记录保留。
- 门户首页显示已发布、未归档资产的累计下载前五名，跟随资产分类筛选。
- 新建和编辑均支持 ZIP、PNG/JPG/GIF/WebP、PDF、文本和源码。单文件上限 10 MB，图片上限 2000 万像素且单边不超过 12000 像素。
- 服务端校验实际内容、路径及图片解码；ZIP 检查中央目录和文件头、CRC、重复路径、符号链接、加密、嵌套压缩包、文件数量、解压大小和压缩比。ZIP 内最多 200 项，解压总量最多 50 MB。
- 下载统一使用附件响应和 `nosniff`；草稿附件和导出只允许负责人或管理员读取。文件内容检查不等同于病毒扫描，不会执行上传的脚本。
